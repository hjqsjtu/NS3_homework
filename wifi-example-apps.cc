...
NS_LOG_COMPONENT_DEFINE ("Wifi-example-apps");
...
Sender::GetTypeId (void)
{
  static TypeId tid = TypeId ("Sender")
    ...
    .AddAttribute ("Interval", "Delay between transmissions.",
                   DoubleValue (0.00002),
                   MakeDoubleAccessor(&Sender::m_interval),
                   MakeDoubleChecker<double>(0.00001,0.5))
...
void
Sender::SetLastTracker(Ptr<TimeMinMaxAvgTotalCalculator> xlast)
{
  m_last = xlast;
...
void
  Receiver::SetJitterTracker (Ptr<MinMaxAvgTotalCalculator<double>> jitter)
{
  m_jitter = jitter;
  //m_jitter->Update(0);
  // end Receiver::SetJitterTracker
}
void
Receiver::Receive (Ptr<Socket> socket)
{
  ...
  while ((packet = socket->RecvFrom (from))) {
      ...
      if (packet->FindFirstMatchingByteTag (timestamp)) {
          ...
          Time delta =Abs( (Simulator::Now () - m_previousRx) - (tx - m_previousRxTx));
          //m_jitter += (Abs (delta) - m_jitter) / 16;
          double pJitter=0;
          if(!bFirstPacket)
            pJitter=m_jitter->getMean();
          else
            bFirstPacket = false;
          double cJitter=delta.ToDouble(Time::US);
          pJitter += (cJitter - pJitter)/16;
          m_jitter->Update(pJitter);
          m_previousRx = Simulator::Now ();
          m_previousRxTx = tx;
