...
do
  for slot in $SLOT
  do
    echo Trial $trial, slot $slot
    sudo ../../waf --run "wifi-timing-attributes --format=db --slot=$slot --run=run-$slot-$trial"
  done
done
...
CMD="select exp.input, \
  ...
  receiver.run=delaydata.run and \
  ...
  receiver.variable='receiver-rx-packets' and \
...
if [ -e wifi-result.data ]
then
    echo Deleting wifi-result.data
    sudo rm -rf wifi-result.data
fi
sqlite3 -noheader mydata.db "$CMD" > wifi-result.data
sed -i.bak "s/|/   /g" wifi-result.data
rm wifi-result.data.bak
gnuplot5 wifi-parameters-slot.gnuplot
